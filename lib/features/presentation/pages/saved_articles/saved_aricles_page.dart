...
class SavedArticles extends HookWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<LocalArticleCubit>()..getArticles(ArticleEntity()),
      child: Scaffold(
        appBar: _buildAppBar(),
        body: _buildBody(),
      ),
  ...
  Widget _buildBody() {
    return BlocBuilder<LocalArticleCubit, LocalArticleState>(
      builder: (context, articlestate) {
        if (articlestate is LocalArticleLoading) {
          return const Center(child: CupertinoActivityIndicator());
        }
        if (articlestate is LocalArticleLoaded) {
          final article = articlestate.news;

          return _buildArticlesList(articlestate.news);
        }
        return Center(child: Text(articlestate.toString()));
      },
    );
  }
  ...
  void _onRemoveArticle(BuildContext context, ArticleEntity article) {
    BlocProvider.of<LocalArticleCubit>(context).removeArticles(article);
    BlocProvider.of<LocalArticleCubit>(context).getArticles(ArticleEntity());
  }
