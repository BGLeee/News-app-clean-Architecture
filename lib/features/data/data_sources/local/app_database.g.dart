...
class _$AppDatabaseBuilder {
  ...
  Future<AppDatabase> build() async {
    ...
    // database.database
    //     .delete('article')
    //     .then((value) => print("deleted your table"));
    database.database = await database.open(
      path,
      _migrations,
      _callback,
    );
    // await File(path).delete().then((value) => print(path));
    // print(path);
    return database;
  }
}
  ...
  Future<sqflite.Database> open(String path, List<Migration> migrations,
    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onOpen: (database) async {
        // await database
        //     .execute(
        //         'CREATE TABLE IF NOT EXISTS `article`(`id` TEXT, `author` TEXT, `title` TEXT, `description` TEXT, `urlToImage` TEXT, `publishedAt` TEXT, `content` TEXT,`articleUrl` TEXT, PRIMARY KEY (`id`))')
        //     .then((value) => print("After the table createiion"));
        // database.delete("article").then((value) => print("it delted"));
    ...
    // database.delete("article").then((value) => print("its been deleted"));
  ...
  Future<List<ArticleModel>> getArticle() async {
    return _queryAdapter.queryList('SELECT * FROM article',
        mapper: (Map<String, Object?> row) => ArticleModel(
            ...
            imageUrl: row['urlToImage'] as String?,
